...
describe PhotoMapper do
  ...
  context 'current_time is same between both files' do
    ...
    xit 'time is in ImageUploader class' do
      @saved_photo = PhotoMapper.new(1)
      @saved_photo.uploader = File.open('app/assets/images/thug_dog2.jpg')
      expect(@saved_photo.uploader.filename).to include(@saved_photo.uploader.current_time.to_s)
    end
    xit 'should have the modified time in the full filename' do
      @saved_photo = PhotoMapper.new(1)
      @saved_photo.uploader = File.open('app/assets/images/thug_dog2.jpg')
      # @saved_photo.save
      current_time = Time.now.to_i
      expect(@saved_photo.filename).to include(@saved_photo.uploader.current_time.to_s)
    end
  end
  ...
  context 'The GET image_url' do
    ...
    xit 'should explicitly match this line with a name as a string' do
      ...
      # @saved_photo.save
      current_time = Time.now.to_i
      expect(@saved_photo.image_url).to match("https://s3.amazonaws.com/bi-portal.dcnd/uploads/#{Rails.env}/photo_mapper/Cool%20Report/thumb_Cool%20Report_#{@saved_photo.uploader.current_time}.png")
    end
    xit 'should explicitly match this line with a name as an integer' do
      ...
      # @saved_photo.save
      current_time = Time.now.to_i
      expect(@saved_photo.image_url).to match("https://s3.amazonaws.com/bi-portal.dcnd/uploads/#{Rails.env}/photo_mapper/1/thumb_1_#{@saved_photo.uploader.current_time}.png")
    end
  end
  ...
  #     expect(@saved_photo.image_url).to have_dimensions(75, 75)
  #   end
